...
fn child_uses_parent_live_then_user_and_project_spawn_cfgs() {
	...
	parent.run("ai_model live; ai_fastmode false; ai_thinking high").expect("parent values");
	let files = CfgFiles::with_roots(user, Some(project));
	let child = child_ctx(&parent, &files, "sonic").expect("child context");

	assert_eq!(child.get_typed::<omp_core::Str>("ai_model").expect("model").as_str(), "live");
	assert!(child.get_typed::<bool>("ai_fastmode").expect("fast mode"));
	assert_eq!(child.get_typed::<omp_core::Str>("ai_thinking").expect("thinking").as_str(), "low");
	assert_eq!(
		parent.get_typed::<omp_core::Str>("ai_model").expect("parent model").as_str(),
		"live"
	);
}
