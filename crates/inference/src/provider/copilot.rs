...
//! accounting (pi `packages/ai/src/providers/github-copilot-headers.ts`).
//!
...
/// Pi `inferCopilotInitiator`: the last message decides. A user message that
...
/// Pi `hasCopilotVisionInput`: any image in a user or tool-result message.
...
/// Pi `getCopilotInitiatorOverride`: a configured `X-Initiator` wins over
/// inference; the last valid value is used.
...
/// Pi `getCopilotPremiumMultiplier`: an undeclared multiplier is `1`, and a
/// free plan bills a `0×` model as one request.
pub fn premium_multiplier(declared: Option<PremiumMultiplier>) -> PremiumMultiplier {
	match declared {
...
/// Pi `getCopilotPremiumRequests`, in `Usage::PREMIUM_REQUEST_SCALE` units.
pub fn premium_requests_millionths(
