		...
		match request {
			DebugRequest::Info => (
				serde_json::json!({
					...
					"height": self.host.frame().size().height,
			...
			DebugRequest::Text => {
				let lines = debug::frame_text(self.host.frame())
					.lines()
					.map(str::to_owned)
					.collect::<Vec<_>>();
				let start = lines
			...
			DebugRequest::Frame => match debug::frame_png(self.host.frame()) {
				Ok(png) => (
					serde_json::json!({
						"ok": true,
						"lines": debug::frame_text(self.host.frame()).lines().collect::<Vec<_>>(),
						"png": png,
						"surface": "native",
					}),
					Effect::Ignored,
